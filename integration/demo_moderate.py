    ...
    try:
        while True:
            ...
            for i in range(n_total):
                ...
                n_done += 1
                overall = health.get("overall_health_score")
                if overall is not None:
                    tally["overall"].append(float(overall))
                if rsul.get("predicted_rsul_hours") is not None:
                    tally["rsul"].append(float(rsul["predicted_rsul_hours"]))
                ...
                print(
                    f"\r  sample {i + 1:>5}/{n_total}  health {float(overall or 0):6.2f}  "
                    f"{str(health.get('health_state', '')):<13} "
                    f"rsul {float(rsul.get('predicted_rsul_hours') or 0):7.2f} h  "
